...
def find_instagram_via_web_search(username=None, collection_slug=None, collection_name=None):
    """Conservative last-resort public-web lookup without DuckDuckGo.

    Railway was repeatedly timing out against DuckDuckGo. This fallback uses
    independent Bing and Yahoo result pages instead. A discovered Instagram
    handle is accepted only when it strongly matches the OpenSea/collection
    identity, preferring false negatives over a wrong creator account.
    """
    identities = []
    ...
    # No DuckDuckGo here: it consistently ConnectTimeouts from Railway.
    providers = [
    ...
    print('INSTAGRAM WEB SEARCH: no verified handle | identities =', identities)
    return None
